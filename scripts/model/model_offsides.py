...
model_offsides.py
=================
Model predykcji SPALONYCH w meczu Ekstraklasy.

Rynki:
- pełny rozkład sumy spalonych w meczu
- Over/Under dla linii:
    0.5, 1.5, 2.5, 3.5, 4.5, 5.5, 6.5

Architektura:
- Poisson
- osobno modelujemy spalone gospodarzy i gości
- rolling features kauzalne dla wszystkich sezonów
- pełny rozkład sumy:
    p_sum_0 ... p_sum_15, p_sum_16_plus
...
OUTPUT_CSV = Path("data/processed/model_offsides_oos_predictions.csv")
REPORT_PATH = Path("data/reports/model/model_offsides_oos_report.txt")

VAL_SEASON = "2024/25"
TEST_SEASON = "2025/26"

OU_LINES = [x + 0.5 for x in range(0, 7)]   # 0.5 ... 6.5
MAX_OFFSIDES = 20
EXACT_SUM_MAX = 15
...
def load_matches():
    ...
    for col in ["spalone_gosp", "spalone_gosc", "kolejka"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")

    df = df.dropna(subset=["spalone_gosp", "spalone_gosc", "kolejka"]).copy()
...
def build_causal_offside_features(df_all, window=8):
    """
    ...
    for _, row in df.iterrows():
        ...
        feat_row = {
            ...
            "gospodarz": home,
            "gosc": away,
            "spalone_gosp_rzecz": float(row["spalone_gosp"]),
            "spalone_gosc_rzecz": float(row["spalone_gosc"]),
            "suma_rzecz": float(row["spalone_gosp"] + row["spalone_gosc"]),
        ...
        home_for = float(row["spalone_gosp"])
        home_against = float(row["spalone_gosc"])
        away_for = float(row["spalone_gosc"])
        away_against = float(row["spalone_gosp"])
...
def fit_poisson_model(df_train):
    """
    log(mu_home) = b_home
    ...
    y_home = df["spalone_gosp_rzecz"].values.astype(float)
    y_away = df["spalone_gosc_rzecz"].values.astype(float)
...
def predict_offsides(model, home_for, away_against, home_total_for, away_for, home_against, away_total_for):
    b_home = model["b_home"]
    b_away = model["b_away"]
    w_for = model["w_for"]
    ...
    p_home_vec = np.array([poisson.pmf(k, mu_home) for k in range(MAX_OFFSIDES + 1)])
    p_away_vec = np.array([poisson.pmf(k, mu_away) for k in range(MAX_OFFSIDES + 1)])

    p_sum = np.convolve(p_home_vec, p_away_vec)[:MAX_OFFSIDES + 1]
    total = p_sum.sum()
    if total <= 0:
        p_sum = np.zeros(MAX_OFFSIDES + 1)
        p_sum[0] = 1.0
    else:
        p_sum = p_sum / total
    ...
    for k in kolejki:
        ...
        for _, mecz in df_round.iterrows():
            ...
            row = {
                ...
                "gosc": mecz["gosc"],
                "spalone_gosp_rzecz": float(mecz["spalone_gosp_rzecz"]),
                "spalone_gosc_rzecz": float(mecz["spalone_gosc_rzecz"]),
...
def main():
    ...
    df_features = build_causal_offside_features(df_all, window=8)
    ...
    lines = []
    lines.append("=" * 78)
    lines.append("MODEL SPALONYCH — OOS RAPORT")
    lines.append("=" * 78)
    lines.append("")
    ...
    lines.append("  Pełny rozkład sumy zapisany jako p_sum_0 ... p_sum_15, p_sum_16_plus")
    ...
    lines.append("-" * 78)
    lines.append(f"  lambda_home mean: {df_test['lambda_home'].mean():.2f} | rzecz: {df_test['spalone_gosp_rzecz'].mean():.2f}")
    lines.append(f"  lambda_away mean: {df_test['lambda_away'].mean():.2f} | rzecz: {df_test['spalone_gosc_rzecz'].mean():.2f}")
