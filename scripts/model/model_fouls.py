...
model_fouls.py
==============
Model predykcji FAULI w meczu Ekstraklasy.

Rynki:
- pełny rozkład sumy fauli w meczu
- Over/Under dla linii:
    15.5, 16.5, 17.5, ..., 33.5

Architektura:
- Negative Binomial
- osobno modelujemy faule gospodarzy i gości
...
Uwaga:
- to jest baseline BEZ sędziego
- później dokładamy warstwę referee-based adjustment
"""
...
OUTPUT_CSV = Path("data/processed/model_fouls_oos_predictions.csv")
REPORT_PATH = Path("data/reports/model/model_fouls_oos_report.txt")
...
def predict_fouls(model, home_for, away_against, home_total_for, away_for, home_against, away_total_for):
    b_home = model["b_home"]
    b_away = model["b_away"]
    w_for = model["w_for"]
...
def main():
    ...
    lines = []
    lines.append("=" * 78)
    lines.append("MODEL FAULI — OOS RAPORT")
    lines.append("=" * 78)
    lines.append("")
    ...
    lines.append("  Pełny rozkład sumy zapisany jako p_sum_0 ... p_sum_45, p_sum_46_plus")
    lines.append(f"  Linie OU: {OU_LINES[0]} ... {OU_LINES[-1]}")
    lines.append("  Rolling window: 8 meczów")
    lines.append("  WERSJA BEZ SĘDZIEGO (baseline)")
    lines.append("")
    ...
    lines.append("-" * 78)
    lines.append(f"  lambda_home mean: {df_test['lambda_home'].mean():.2f} | rzecz: {df_test['faule_gosp_rzecz'].mean():.2f}")
    lines.append(f"  lambda_away mean: {df_test['lambda_away'].mean():.2f} | rzecz: {df_test['faule_gosc_rzecz'].mean():.2f}")
