...
model_shots.py
==============
Model predykcji STRZAŁÓW w meczu Ekstraklasy.

Rynki:
- pełny rozkład sumy strzałów w meczu
- Over/Under dla linii:
    12.5, 13.5, 14.5, ..., 35.5

Architektura:
- Negative Binomial
- osobno modelujemy strzały gospodarzy i gości
- rolling features kauzalne dla wszystkich sezonów
- pełny rozkład sumy:
    p_sum_0 ... p_sum_50, p_sum_51_plus
...
OUTPUT_CSV = Path("data/processed/model_shots_oos_predictions.csv")
REPORT_PATH = Path("data/reports/model/model_shots_oos_report.txt")

VAL_SEASON = "2024/25"
TEST_SEASON = "2025/26"

OU_LINES = [x + 0.5 for x in range(12, 36)]   # 12.5 ... 35.5
MAX_SHOTS = 70
EXACT_SUM_MAX = 50
...
def load_matches():
    ...
    for col in ["strzaly_gosp", "strzaly_gosc", "kolejka"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")

    df = df.dropna(subset=["strzaly_gosp", "strzaly_gosc", "kolejka"]).copy()
...
def build_causal_shot_features(df_all, window=8):
    """
    ...
    for _, row in df.iterrows():
        ...
        feat_row = {
            ...
            "gospodarz": home,
            "gosc": away,
            "strzaly_gosp_rzecz": float(row["strzaly_gosp"]),
            "strzaly_gosc_rzecz": float(row["strzaly_gosc"]),
            "suma_rzecz": float(row["strzaly_gosp"] + row["strzaly_gosc"]),
        ...
        home_for = float(row["strzaly_gosp"])
        home_against = float(row["strzaly_gosc"])
        away_for = float(row["strzaly_gosc"])
        away_against = float(row["strzaly_gosp"])
...
def fit_negbin_model(df_train):
    ...
    y_home = df["strzaly_gosp_rzecz"].values.astype(int)
    y_away = df["strzaly_gosc_rzecz"].values.astype(int)
    ...
    x0 = np.array([
        1.95,           # exp(1.95)=7.03 -> po feature'ach wyjdzie sensowna skala
        1.75,
        0.30,
        0.20,
        0.15,
...
def predict_shots(model, home_for, away_against, home_total_for, away_for, home_against, away_total_for):
    b_home = model["b_home"]
    b_away = model["b_away"]
    w_for = model["w_for"]
    ...
    p_home_vec = np.array([nbinom.pmf(k, r, p_h) for k in range(MAX_SHOTS + 1)])
    p_away_vec = np.array([nbinom.pmf(k, r, p_a) for k in range(MAX_SHOTS + 1)])

    p_sum = np.convolve(p_home_vec, p_away_vec)[:MAX_SHOTS + 1]
    total = p_sum.sum()
    if total <= 0:
        p_sum = np.zeros(MAX_SHOTS + 1)
        p_sum[0] = 1.0
    else:
        p_sum = p_sum / total
    ...
    for k in kolejki:
        ...
        for _, mecz in df_round.iterrows():
            ...
            row = {
                ...
                "gosc": mecz["gosc"],
                "strzaly_gosp_rzecz": float(mecz["strzaly_gosp_rzecz"]),
                "strzaly_gosc_rzecz": float(mecz["strzaly_gosc_rzecz"]),
...
def main():
    ...
    df_features = build_causal_shot_features(df_all, window=8)
    ...
    lines = []
    lines.append("=" * 78)
    lines.append("MODEL STRZAŁÓW — OOS RAPORT")
    lines.append("=" * 78)
    lines.append("")
    ...
    lines.append("  Pełny rozkład sumy zapisany jako p_sum_0 ... p_sum_50, p_sum_51_plus")
    ...
    lines.append("-" * 78)
    lines.append(f"  lambda_home mean: {df_test['lambda_home'].mean():.2f} | rzecz: {df_test['strzaly_gosp_rzecz'].mean():.2f}")
    lines.append(f"  lambda_away mean: {df_test['lambda_away'].mean():.2f} | rzecz: {df_test['strzaly_gosc_rzecz'].mean():.2f}")
