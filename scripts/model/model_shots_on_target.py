"""
model_shots_on_target.py
========================
Model predykcji STRZAŁÓW CELNYCH w meczu Ekstraklasy.

Rynki:
- pełny rozkład sumy strzałów celnych w meczu
- Over/Under dla linii:
    3.5, 4.5, 5.5, ..., 15.5

Architektura:
- Poisson
- osobno modelujemy strzały celne gospodarzy i gości
- rolling features kauzalne dla wszystkich sezonów
- pełny rozkład sumy:
    p_sum_0 ... p_sum_25, p_sum_26_plus
...
OUTPUT_CSV = Path("data/processed/model_shots_on_target_oos_predictions.csv")
REPORT_PATH = Path("data/reports/model/model_shots_on_target_oos_report.txt")

VAL_SEASON = "2024/25"
TEST_SEASON = "2025/26"

OU_LINES = [x + 0.5 for x in range(3, 16)]   # 3.5 ... 15.5
MAX_SOT = 30
EXACT_SUM_MAX = 25
...
def load_matches():
    ...
    for col in ["celne_gosp", "celne_gosc", "kolejka"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")

    df = df.dropna(subset=["celne_gosp", "celne_gosc", "kolejka"]).copy()
...
def build_causal_sot_features(df_all, window=8):
    """
    ...
    for _, row in df.iterrows():
        ...
        feat_row = {
            ...
            "gospodarz": home,
            "gosc": away,
            "celne_gosp_rzecz": float(row["celne_gosp"]),
            "celne_gosc_rzecz": float(row["celne_gosc"]),
            "suma_rzecz": float(row["celne_gosp"] + row["celne_gosc"]),
        ...
        home_for = float(row["celne_gosp"])
        home_against = float(row["celne_gosc"])
        away_for = float(row["celne_gosc"])
        away_against = float(row["celne_gosp"])
...
def fit_poisson_model(df_train):
    ...
    y_home = df["celne_gosp_rzecz"].values.astype(float)
    y_away = df["celne_gosc_rzecz"].values.astype(float)
...
def predict_sot(model, home_for, away_against, home_total_for, away_for, home_against, away_total_for):
    b_home = model["b_home"]
    b_away = model["b_away"]
    w_for = model["w_for"]
    ...
    p_home_vec = np.array([poisson.pmf(k, mu_home) for k in range(MAX_SOT + 1)])
    p_away_vec = np.array([poisson.pmf(k, mu_away) for k in range(MAX_SOT + 1)])

    p_sum = np.convolve(p_home_vec, p_away_vec)[:MAX_SOT + 1]
    total = p_sum.sum()
    if total <= 0:
        p_sum = np.zeros(MAX_SOT + 1)
        p_sum[0] = 1.0
    else:
        p_sum = p_sum / total
    ...
    for k in kolejki:
        ...
        for _, mecz in df_round.iterrows():
            ...
            row = {
                ...
                "gosc": mecz["gosc"],
                "celne_gosp_rzecz": float(mecz["celne_gosp_rzecz"]),
                "celne_gosc_rzecz": float(mecz["celne_gosc_rzecz"]),
...
def main():
    ...
    df_features = build_causal_sot_features(df_all, window=8)
    ...
    lines = []
    lines.append("=" * 78)
    lines.append("MODEL STRZAŁÓW CELNYCH — OOS RAPORT")
    lines.append("=" * 78)
    lines.append("")
    ...
    lines.append("  Pełny rozkład sumy zapisany jako p_sum_0 ... p_sum_25, p_sum_26_plus")
    ...
    lines.append("-" * 78)
    lines.append(f"  lambda_home mean: {df_test['lambda_home'].mean():.2f} | rzecz: {df_test['celne_gosp_rzecz'].mean():.2f}")
    lines.append(f"  lambda_away mean: {df_test['lambda_away'].mean():.2f} | rzecz: {df_test['celne_gosc_rzecz'].mean():.2f}")
