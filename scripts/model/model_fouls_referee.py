"""
model_fouls_referee.py
======================
Challenger dla modelu FAULI w meczu Ekstraklasy.

Cel:
- zachowac ta sama metodologie co oficjalny baseline model_fouls.py
- dolozyc tylko aspekt sedziego
- porownac baseline vs referee w identycznym rolling OOS

Rynki:
- pelny rozklad sumy fauli w meczu
- Over/Under dla linii:
    15.5, 16.5, ..., 33.5

Architektura:
- Negative Binomial
- osobno modelujemy faule gospodarzy i gosci
- rolling features kauzalne dla wszystkich sezonow
- pelny rozklad sumy:
    p_sum_0 ... p_sum_45, p_sum_46_plus
...
- dodatkowy feature:
    ref_log_ratio = log(ref_fouls_shrunk_before + 0.5) - log(league_fouls_avg_before + 0.5)

Feature refereego dziala symetrycznie na obie strony:
- log(mu_home) ... + w_ref * ref_log_ratio
- log(mu_away) ... + w_ref * ref_log_ratio
"""
...
OUTPUT_CSV = ROOT / "data" / "processed" / "model_fouls_referee_oos_predictions.csv"
REPORT_PATH = ROOT / "data" / "reports" / "model" / "model_fouls_referee_oos_report.txt"

VAL_SEASON = "2024/25"
TEST_SEASON = "2025/26"

OU_LINES = [x + 0.5 for x in range(15, 34)]   # 15.5 ... 33.5
MAX_FAULS = 60
EXACT_SUM_MAX = 45
...
def load_matches():
    ...
    for col in ["faule_gosp", "faule_gosc", "kolejka"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")

    df = df.dropna(subset=["faule_gosp", "faule_gosc", "kolejka"]).copy()
...
def build_causal_foul_features(df_all, window=8):
    """
    Buduje rolling features mecz po meczu, bez leakage.
    To jest ta sama logika co w baseline model_fouls.py
    """
    df = df_all.copy()
    ...
    for _, row in df.iterrows():
        ...
        feat_row = {
            ...
            "gospodarz": home,
            "gosc": away,
            "faule_gosp_rzecz": float(row["faule_gosp"]),
            "faule_gosc_rzecz": float(row["faule_gosc"]),
            "suma_rzecz": float(row["faule_gosp"] + row["faule_gosc"]),
        ...
        home_for = float(row["faule_gosp"])
        home_against = float(row["faule_gosc"])
        away_for = float(row["faule_gosc"])
        away_against = float(row["faule_gosp"])
...
def merge_referee_features(df_features, df_ref):
    ...
    df["league_fouls_avg_before"] = pd.to_numeric(df["league_fouls_avg_before"], errors="coerce")
    df["ref_fouls_shrunk_before"] = pd.to_numeric(df["ref_fouls_shrunk_before"], errors="coerce")
    ...
    if df["league_fouls_avg_before"].isna().any() or df["ref_fouls_shrunk_before"].isna().any():
        raise RuntimeError("Po merge referee features sa NaN w kolumnach fouls avg before.")

    df["ref_log_ratio"] = (
        np.log(df["ref_fouls_shrunk_before"].astype(float) + 0.5)
        - np.log(df["league_fouls_avg_before"].astype(float) + 0.5)
    )

    df["ref_delta_raw"] = df["ref_fouls_shrunk_before"] - df["league_fouls_avg_before"]
...
def fit_negbin_model(df_train, use_referee=False):
    req_cols = [
        "home_for_avg",
        "home_against_avg",
        "away_for_avg",
    ...
    if use_referee:
        req_cols.append("ref_log_ratio")

    df = df_train.copy().dropna(subset=req_cols)

    y_home = df["faule_gosp_rzecz"].values.astype(int)
    y_away = df["faule_gosc_rzecz"].values.astype(int)
    ...
    if use_referee:
        x_ref = np.clip(df["ref_log_ratio"].values.astype(float), -1.0, 1.0)

        def nll(params):
            b_home, b_away, w_for, w_against, w_total, w_ref, log_alpha = params
        ...
        b_home, b_away, w_for, w_against, w_total, w_ref, log_alpha = result.x
...
def predict_fouls(
    ...
    x_ref = float(np.clip(ref_log_ratio, -1.0, 1.0)) if model.get("use_referee", False) else 0.0
    ...
    p_home_vec = np.array([nbinom.pmf(k, r, p_h) for k in range(MAX_FAULS + 1)])
    p_away_vec = np.array([nbinom.pmf(k, r, p_a) for k in range(MAX_FAULS + 1)])

    p_sum = np.convolve(p_home_vec, p_away_vec)[:MAX_FAULS + 1]
    total = p_sum.sum()
    if total <= 0:
        p_sum = np.zeros(MAX_FAULS + 1)
        p_sum[0] = 1.0
    else:
        p_sum = p_sum / total
...
def run_backtesting(df_features, target_season, use_referee=False, label="baseline"):
    ...
    for k in kolejki:
        ...
        model = fit_negbin_model(df_train, use_referee=use_referee)
        ...
        for _, mecz in df_round.iterrows():
            pred = predict_fouls(
                ...
                ref_log_ratio=float(mecz["ref_log_ratio"]) if use_referee else 0.0,
            )
            ...
            row = {
                ...
                "gosc": mecz["gosc"],
                "faule_gosp_rzecz": float(mecz["faule_gosp_rzecz"]),
                "faule_gosc_rzecz": float(mecz["faule_gosc_rzecz"]),
                ...
                "league_fouls_avg_before": float(mecz["league_fouls_avg_before"]),
                "ref_fouls_shrunk_before": float(mecz["ref_fouls_shrunk_before"]),
                "ref_delta_raw": float(mecz["ref_delta_raw"]),
                "ref_log_ratio": float(mecz["ref_log_ratio"]),
...
def main():
    ...
    df_features = build_causal_foul_features(df_all, window=8)
    ...
    lines = []
    lines.append("=" * 100)
    lines.append("MODEL FAULI - REFEREE CHALLENGER OOS RAPORT")
    lines.append("=" * 100)
    lines.append("")
    lines.append("1. ARCHITEKTURA")
    lines.append("-" * 100)
    lines.append("  Ta sama metodologia co oficjalny baseline model_fouls.py")
    ...
    lines.append("      + w_ref * ref_log_ratio")
    lines.append("  gdzie:")
    lines.append("      ref_log_ratio = log(ref_fouls_shrunk_before + 0.5) - log(league_fouls_avg_before + 0.5)")
    lines.append("  Pelny rozklad sumy zapisany jako p_sum_0 ... p_sum_45, p_sum_46_plus")
    ...
    lines.append("-" * 100)
    lines.append(f"  lambda_home mean: {df_test_ref['lambda_home'].mean():.2f} | rzecz: {df_test_ref['faule_gosp_rzecz'].mean():.2f}")
    lines.append(f"  lambda_away mean: {df_test_ref['lambda_away'].mean():.2f} | rzecz: {df_test_ref['faule_gosc_rzecz'].mean():.2f}")
    lines.append(f"  lambda_suma mean: {df_test_ref['lambda_suma'].mean():.2f} | rzecz: {df_test_ref['suma_rzecz'].mean():.2f}")
    lines.append(f"  alpha mean:       {df_test_ref['alpha'].mean():.4f}")
    ...
    lines.append(f"  ref_fouls_shrunk_before mean: {df_test_ref['ref_fouls_shrunk_before'].mean():.3f}")
    lines.append(f"  league_fouls_avg_before mean: {df_test_ref['league_fouls_avg_before'].mean():.3f}")
    lines.append(f"  ref_delta_raw mean:           {df_test_ref['ref_delta_raw'].mean():.3f}")
    lines.append(f"  ref_log_ratio mean:           {df_test_ref['ref_log_ratio'].mean():.4f}")
