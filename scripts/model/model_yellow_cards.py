"""
model_yellow_cards.py
=====================
Model predykcji ŻÓŁTYCH KARTEK w meczu Ekstraklasy.

Rynki:
- pełny rozkład sumy żółtych kartek w meczu
- Over/Under dla linii:
    0.5, 1.5, 2.5, ..., 7.5

Architektura:
- Poisson
- osobno modelujemy żółte kartki gospodarzy i gości
...
OUTPUT_CSV = Path("data/processed/model_yellow_cards_oos_predictions.csv")
REPORT_PATH = Path("data/reports/model/model_yellow_cards_oos_report.txt")
...
def predict_yc(model, home_for, away_against, home_total_for, away_for, home_against, away_total_for):
    b_home = model["b_home"]
    b_away = model["b_away"]
    w_for = model["w_for"]
...
def main():
    ...
    lines = []
    lines.append("=" * 78)
    lines.append("MODEL ŻÓŁTYCH KARTEK — OOS RAPORT")
    lines.append("=" * 78)
    lines.append("")
    ...
    lines.append("  Pełny rozkład sumy zapisany jako p_sum_0 ... p_sum_12, p_sum_13_plus")
    ...
    lines.append("-" * 78)
    lines.append(f"  lambda_home mean: {df_test['lambda_home'].mean():.2f} | rzecz: {df_test['zk_gosp_rzecz'].mean():.2f}")
    lines.append(f"  lambda_away mean: {df_test['lambda_away'].mean():.2f} | rzecz: {df_test['zk_gosc_rzecz'].mean():.2f}")
